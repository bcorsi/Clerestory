      ...
      <div className="main-area">
        ...
        <div className="main-content" onClick={() => { setShowSearch(false); setSearchResults(null); }}>
          {loading ? (
            ...
            {page === 'deal-detail' && selectedDeal && <DealDetail deal={selectedDeal} activities={activities} tasks={tasks} properties={properties} contacts={contacts} accounts={accounts} leaseComps={leaseComps} saleComps={saleComps} notes={notes} followUps={followUps} onRefresh={loadData} showToast={showToast} onPropertyClick={openProperty} onContactClick={openContact} onAccountClick={openAccount} onCatalystClick={openCatalyst} onAddActivity={(leadId, dealId) => setModal({ type: 'add-activity', defaultDealId: dealId })} onAddTask={(dealId) => setModal({ type: 'add-task', defaultDealId: dealId })} onLeaseCompClick={openLeaseComp} />}
