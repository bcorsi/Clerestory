...
const PARCEL_COLOR = '#00C2FF';   // bright cyan — visible on satellite
...
export default function AerialThumbnail({ address, city, apns, parcelGeometry, latitude, longitude, height = 280, zoom = 17 }) {
  ...
  const storedGeoStr = hasStoredGeometry
    ? JSON.stringify(parcelGeometry).replace(/\\/g, '\\\\').replace(/'/g, "\\'").replace(/<\//g, '<\\/')
    : '';
  ...
  const center = (latitude && longitude) ? [parseFloat(latitude), parseFloat(longitude)] : COORDS[c] || [34.0, -117.8];
...
.lbl{position:absolute;bottom:8px;left:8px;z-index:1000;background:rgba(18,24,36,0.82);border-radius:5px;padding:4px 10px;font-family:'Instrument Sans',sans-serif;font-size:11px;color:rgba(240,235,225,0.88);backdrop-filter:blur(4px);pointer-events:none}
</style>
</head><body>
<div id="map"></div>
<div class="lbl" id="lbl">${hasStoredGeometry ? safeAddr : 'Loading...'}</div>
<script>
...
(function(){
  ...
  try {
    ...
    lbl.textContent=data.features.length+' parcel'+(data.features.length>1?'s':'')+' \u00B7 ${safeAddr}';
  } catch(e){ lbl.textContent='${safeAddr}'; }
})();
` : `
${latitude && longitude ? `queryParcels(${parseFloat(latitude)},${parseFloat(longitude)});` : `
var geocodeUrl='https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/findAddressCandidates?f=json&outSR=4326&maxLocations=1&singleLine='+encodeURIComponent('${query.replace(/\\/g,'\\\\').replace(/'/g,"\\'")}');
...
function queryParcels(lat,lng){
  ...
  fetch(url).then(function(r){return r.json()}).then(function(data){
    if(data.features&&data.features.length>0) drawParcels(data,lbl);
    else {
      ...
      fetch(sbUrl).then(function(r){return r.json()}).then(function(d2){
        if(d2.features&&d2.features.length>0) drawParcels(d2,lbl); else pointQuery(lat,lng,lbl);
      }).catch(function(){pointQuery(lat,lng,lbl);});
    }
  }).catch(function(){pointQuery(lat,lng,lbl);});
  ` : `pointQuery(lat,lng,lbl);`}
}

function pointQuery(lat,lng,lbl){
  ...
  fetch(url).then(function(r){return r.json()}).then(function(data){
    if(data.features&&data.features.length>0) drawParcels(data,lbl);
      ...
      fetch(sbUrl).then(function(r){return r.json()}).then(function(d2){
        if(d2.features&&d2.features.length>0) drawParcels(d2,lbl); else lbl.textContent='${safeAddr}';
      }).catch(function(){lbl.textContent='${safeAddr}';});
    }
  }).catch(function(){lbl.textContent='${safeAddr}';});
}

function drawParcels(data,lbl){
  var group=L.geoJSON(data,{style:${parcelStyle}}).addTo(map);
  map.fitBounds(group.getBounds(),{padding:[20,20],maxZoom:18});
  lbl.textContent=data.features.length+' parcel'+(data.features.length>1?'s':'')+' \u00B7 ${safeAddr}';
}
