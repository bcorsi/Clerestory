    ...
    return (
      <div onClick={() => setExpanded(open ? null : lead.id)} onDoubleClick={() => onLeadClick?.(lead)}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
          <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)', flex: 1, marginRight: '8px' }}>{lead.lead_name}</span>
          <div style={{ display: 'flex', gap: '3px', flexShrink: 0 }}>
            {lead.tier && <span style={{ fontSize: '10px', fontWeight: 700, padding: '2px 5px', borderRadius: '3px', background: tierColor(lead.tier) + '22', color: tierColor(lead.tier) }}>{lead.tier}</span>}
            {lead.score != null && <span style={{ fontSize: '10px', fontFamily: 'var(--font-mono)', color: 'var(--accent)', background: 'var(--accent-soft)', padding: '2px 5px', borderRadius: '3px' }}>{lead.score}</span>}
          </div>
        </div>

        {lead.address && <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '5px' }}>{lead.address}{lead.submarket ? ` · ${lead.submarket}` : ''}</div>}
        ...
        {subs.length > 0 && (
          <div style={{ marginBottom: '6px' }}>
            ...
            <div style={{ fontSize: '10px', color: 'var(--text-muted)', marginTop: '2px' }}>{done}/{subs.length} steps</div>
          </div>
        )}

        {aiStep[lead.id]
          ? <div style={{ fontSize: '11px', color: 'var(--amber)', fontWeight: 500, marginBottom: '5px' }}>✦ {aiStep[lead.id]}</div>
          : lead.next_action && <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '5px' }}>→ {lead.next_action}</div>}

        {lead.catalyst_tags?.length > 0 && (
          <div style={{ display: 'flex', gap: '3px', flexWrap: 'wrap' }}>
            {lead.catalyst_tags.slice(0, 2).map((t) => <span key={t} className="tag tag-ghost" style={{ fontSize: '10px' }}>{t}</span>)}
          </div>
        )}
          ...
          <div style={{ borderTop: '1px solid var(--border-subtle)', paddingTop: '10px', marginTop: '8px' }} onClick={(e) => e.stopPropagation()}>
            {subs.length > 0 && (
              <div style={{ marginBottom: '12px' }}>
                <div style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--text-muted)', marginBottom: '6px' }}>Substeps</div>
                {subs.map((step) => {
                  ...
                  return (
                    <div key={step} onClick={() => toggleSub(lead.id, step)} style={{ display: 'flex', gap: '8px', alignItems: 'center', padding: '3px 0', cursor: 'pointer' }}>
                      <div style={{ width: '14px', height: '14px', borderRadius: '3px', flexShrink: 0, border: '2px solid', borderColor: checked ? 'var(--accent)' : 'var(--border)', background: checked ? 'var(--accent)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '9px', fontWeight: 700 }}>{checked ? '✓' : ''}</div>
                      <span style={{ fontSize: '12px', color: checked ? 'var(--text-muted)' : 'var(--text-secondary)', textDecoration: checked ? 'line-through' : 'none' }}>{step}</span>
                    </div>
                  );
                })}
              </div>
            ...
            {lead.decision_maker && <div style={{ fontSize: '12px', marginBottom: '2px' }}><span style={{ color: 'var(--text-muted)' }}>DM: </span>{lead.decision_maker}</div>}
            {lead.phone && <div style={{ fontSize: '12px', fontFamily: 'var(--font-mono)', color: 'var(--text-secondary)', marginBottom: '2px' }}>{lead.phone}</div>}
            {lead.notes && <div style={{ fontSize: '11px', color: 'var(--text-secondary)', lineHeight: 1.5, margin: '6px 0 10px' }}>{lead.notes.slice(0, 180)}{lead.notes.length > 180 ? '...' : ''}</div>}

            <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', marginBottom: '6px' }}>
              <button className="btn btn-ghost btn-sm" style={{ fontSize: '10px', color: 'var(--amber)', borderColor: 'var(--amber)' }} onClick={(e) => handleAI(lead, e)} disabled={aiLoading === lead.id}>
                {aiLoading === lead.id ? '⟳ Thinking...' : '✦ AI Next Step'}
              </button>
              {LEAD_STAGES.filter((s) => s !== lead.stage).map((s) => (
                <button key={s} className="btn btn-ghost btn-sm" style={{ fontSize: '10px' }} onClick={(e) => handleStage(lead, s, e)}>→ {s}</button>
              ))}
              <button className="btn btn-primary btn-sm" style={{ fontSize: '10px', marginLeft: 'auto' }} disabled={converting === lead.id} onClick={(e) => handleConvert(lead, e)}>
                {converting === lead.id ? '...' : '⚡ Convert'}
              </button>
            </div>
            <button className="btn btn-ghost btn-sm" style={{ fontSize: '10px', width: '100%' }} onClick={() => onLeadClick?.(lead)}>Open Full Page →</button>
          </div>
        )}
      </div>
    );
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
          {LEAD_STAGES.map((s) => (
            <div key={s} style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '12px', color: 'var(--text-muted)' }}>
            ...
            <tbody>
              {active.map((lead) => (
                <tr key={lead.id} onClick={() => onLeadClick?.(lead)} style={{ cursor: 'pointer' }}>
                  <td><div style={{ fontWeight: 500 }}>{lead.lead_name}</div>{lead.address && <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{lead.address}</div>}</td>
                  <td><span style={{ fontSize: '11px', padding: '2px 7px', borderRadius: '4px', background: (LEAD_STAGE_COLORS[lead.stage] || '#6b7280') + '22', color: LEAD_STAGE_COLORS[lead.stage] || '#6b7280', fontWeight: 600 }}>{lead.stage}</span></td>
                  <td>{lead.tier && <span style={{ fontWeight: 700, color: tierColor(lead.tier) }}>{lead.tier}</span>}</td>
                  <td style={{ fontFamily: 'var(--font-mono)', fontSize: '12px', color: 'var(--accent)' }}>{lead.score ?? '—'}</td>
                  <td style={{ fontSize: '12px' }}>{lead.decision_maker || '—'}</td>
                  <td style={{ fontFamily: 'var(--font-mono)', fontSize: '11px' }}>{lead.phone || '—'}</td>
                  <td style={{ fontSize: '12px', color: 'var(--amber)' }}>{lead.next_action || '—'}</td>
                  <td>{lead.priority && <span className={`tag ${lead.priority === 'High' ? 'tag-amber' : 'tag-ghost'}`} style={{ fontSize: '10px' }}>{lead.priority}</span>}</td>
                </tr>
              ))}
          ...
          {LEAD_STAGES.map((stage) => (
            <div key={stage}>
              <div style={{ padding: '8px 12px', borderRadius: 'var(--radius-sm)', background: 'var(--bg-card)', borderTop: `3px solid ${LEAD_STAGE_COLORS[stage]}`, marginBottom: '8px', display: 'flex', justifyContent: 'space-between' }}>
                ...
                <span style={{ fontSize: '11px', fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>{byStage[stage]?.length || 0}</span>
              </div>
              {(byStage[stage] || []).map((lead) => <Card key={lead.id} lead={lead} />)}
              {!(byStage[stage]?.length) && <div style={{ padding: '16px', textAlign: 'center', color: 'var(--text-muted)', fontSize: '12px', border: '1px dashed var(--border)', borderRadius: 'var(--radius)' }}>Empty</div>}
            </div>
          ))}
        </div>
      )}
