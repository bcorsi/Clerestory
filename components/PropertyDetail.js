...
import { updateRow, insertRow, addApn, removeApn, addBuilding, removeBuilding, calculateProbability, setCadence } from '../lib/db';
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
          <div>
            ...
            <div style={{display:'flex',alignItems:'center',gap:'8px',flexWrap:'wrap'}}>
              ...
              {p.address&&<a href={`https://www.google.com/maps/@?api=1&map_action=map&basemap=satellite&zoom=18&center=${encodeURIComponent(p.address+', '+(p.city||'')+', CA')}`} target="_blank" rel="noopener noreferrer" style={{fontSize:'12px',color:'var(--accent)',textDecoration:'none',padding:'2px 8px',borderRadius:'4px',border:'1px solid var(--border)',background:'var(--bg-input)'}}>🛰 Aerial ↗</a>}
