...
export async function addApn(propertyId, apn, acres) {
  if (!isConfigured()) return { id: `apn-${Date.now()}`, property_id: propertyId, apn, acres };
  const { data, error } = await supabase
    .from('property_apns')
    .insert({ property_id: propertyId, apn, acres: acres || null })
    .select()
    .single();
  if (error) throw error;
  return data;
...
export async function removeApn(apnId) {
  if (!isConfigured()) return true;
  const { error } = await supabase.from('property_apns').delete().eq('id', apnId);
  if (error) throw error;
  return true;
}

export async function fetchApns(propertyId) {
...
export async function syncPropertyApns(propertyId, apns) {
  // Replace all APNs for a property (used by EditPropertyModal)
  if (!isConfigured()) return apns;
  // Delete existing
  await supabase.from('property_apns').delete().eq('property_id', propertyId);
  // Insert new
  if (apns.length > 0) {
    const rows = apns.filter(a => a.apn?.trim()).map(a => ({
      property_id: propertyId, apn: a.apn.trim(), acres: a.acres ? parseFloat(a.acres) : null,
    }));
    if (rows.length > 0) {
      const { error } = await supabase.from('property_apns').insert(rows);
      if (error) throw error;
    }
  }
  return apns;
...
export async function fetchFollowUps() {
  ...
  if (error) { console.error('fetchFollowUps error:', error); return []; }
  return data || [];
}
