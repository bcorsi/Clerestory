...
// Convert empty strings to null so Supabase doesn't reject '' for int/numeric/date/uuid columns
function cleanPayload(obj) {
  const cleaned = {};
  for (const [k, v] of Object.entries(obj)) {
    if (v === '' || v === undefined) cleaned[k] = null;
    else if (Array.isArray(v)) cleaned[k] = v;
    else cleaned[k] = v;
  }
  return cleaned;
}
...
export async function insertRow(table, row) {
  // Clean empty strings → null, then normalize
  const clean = cleanPayload(row);
  const normalized = (table === 'properties' || table === 'leads' || table === 'lease_comps' || table === 'sale_comps')
    ? normalizeRecord(clean) : clean;

  // Auto-score leads on create
  if (table === 'leads') {
    const { score, tier, probability } = scoreRecord(normalized);
...
export async function updateRow(table, id, updates) {
  const clean = cleanPayload(updates);

  // Auto-score leads when scoring-relevant fields change
  if (table === 'leads') {
    const scoringFields = ['catalyst_tags', 'vacancy_status', 'lease_expiration', 'last_transfer_date'];
    const hasScoringChange = scoringFields.some(f => f in clean);
    if (hasScoringChange) {
      // Fetch current record to merge with updates before scoring
      let current = {};
      if (isConfigured()) {
        const { data } = await supabase.from('leads').select('catalyst_tags,vacancy_status,lease_expiration,last_transfer_date').eq('id', id).single();
        current = data || {};
      }
      const merged = { ...current, ...clean };
      const { score, tier, probability } = scoreRecord(merged);
      clean.score = score;
      clean.tier = tier;
  ...
  if (!isConfigured()) return { id, ...clean };
...
export async function insertProperty(propData, apns = []) {
  const clean = cleanPayload(propData);
  const normalized = normalizeRecord(clean);

  if (!isConfigured()) {
    return { ...normalized, id: `demo-${Date.now()}`, apns, created_at: new Date().toISOString() };
  }
  ...
  if (apns.length > 0) {
    const apnRows = apns.map((a) => ({ property_id: prop.id, apn: a.apn, acres: a.acres ? parseFloat(a.acres) : null }));
  ...
  if (normalized.address) {
    fetchAerialImage(prop.id, normalized.address, normalized.city).catch(console.error);
  }

  // Auto-fetch parcel geometry in background
  const apnStrings = apns.map(a => a.apn).filter(Boolean);
  fetchParcelGeometry(prop.id, apnStrings, normalized.address, normalized.city).catch(console.error);
...
// ─── PARCEL GEOMETRY (ArcGIS) ────────────────────────────────

const LA_PARCEL_URL = 'https://arcgis.lacounty.gov/arcgis/rest/services/LACounty_Cache/LACounty_Parcel/MapServer/0/query';
const SB_PARCEL_URL = 'https://gis.sbcounty.gov/arcgis/rest/services/Parcels/MapServer/0/query';
const GEOCODE_URL = 'https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/findAddressCandidates';

/**
 * Fetch parcel polygon geometry from county ArcGIS and store on property.
 * Strategy: try APN query first (LA County → SB County), fall back to address geocode → spatial query.
 * Stores GeoJSON FeatureCollection + geocoded lat/lng.
 */
export async function fetchParcelGeometry(propertyId, apns = [], address, city) {
  if (!isConfigured()) return null;

  let geoJson = null;
  let lat = null, lng = null;

  // Step 1: Try APN-based query
  const cleanApns = apns.map(a => (a || '').replace(/-/g, '').trim()).filter(Boolean);
  if (cleanApns.length > 0) {
    const where = cleanApns.map(a => `APN='${a}'`).join(' OR ');

    // Try LA County
    try {
      const url = `${LA_PARCEL_URL}?where=${encodeURIComponent(where)}&outFields=APN,AIN,SitusAddress&returnGeometry=true&f=geojson&outSR=4326`;
      const res = await fetch(url);
      const data = await res.json();
      if (data?.features?.length > 0) geoJson = data;
    } catch (e) { console.error('LA parcel query error:', e); }

    // Try SB County if LA didn't find it
    if (!geoJson) {
      try {
        const url = `${SB_PARCEL_URL}?where=${encodeURIComponent(where)}&outFields=APN&returnGeometry=true&f=geojson&outSR=4326`;
        const res = await fetch(url);
        const data = await res.json();
        if (data?.features?.length > 0) geoJson = data;
      } catch (e) { console.error('SB parcel query error:', e); }
    }
  }

  // Step 2: Geocode address for lat/lng (and as fallback for parcel query)
  if (address) {
    try {
      const q = `${address}, ${city || ''}, CA`;
      const url = `${GEOCODE_URL}?f=json&outSR=4326&maxLocations=1&singleLine=${encodeURIComponent(q)}`;
      const res = await fetch(url);
      const data = await res.json();
    ...
    // Step 3: If no APN results, try spatial query at geocoded point
    if (!geoJson && lat && lng) {
      const geom = JSON.stringify({ x: lng, y: lat, spatialReference: { wkid: 4326 } });

      // Try LA County spatial
      try {
        const url = `${LA_PARCEL_URL}?geometry=${encodeURIComponent(geom)}&geometryType=esriGeometryPoint&spatialRel=esriSpatialRelIntersects&outFields=APN,AIN,SitusAddress&returnGeometry=true&f=geojson&outSR=4326&inSR=4326`;
        const res = await fetch(url);
        const data = await res.json();
        if (data?.features?.length > 0) geoJson = data;
      } catch (e) { console.error('LA spatial query error:', e); }

      // Try SB County spatial
      if (!geoJson) {
        try {
          const url = `${SB_PARCEL_URL}?geometry=${encodeURIComponent(geom)}&geometryType=esriGeometryPoint&spatialRel=esriSpatialRelIntersects&outFields=APN&returnGeometry=true&f=geojson&outSR=4326&inSR=4326`;
          const res = await fetch(url);
          const data = await res.json();
          if (data?.features?.length > 0) geoJson = data;
        } catch (e) { console.error('SB spatial query error:', e); }
      }
    }
  }
  ...
  const updates = {};
  if (geoJson) updates.parcel_geometry = geoJson;
  ...
  if (Object.keys(updates).length > 0) {
    ...
    if (error) console.error('Parcel geometry save error:', error);
  }
...
export async function setCadence(table, recordId, cadenceLabel, cadenceDays) {
  ...
  if (error) throw error;
  // Update cadence on record
  await supabase.from(table).update({ follow_up_cadence: cadenceLabel }).eq('id', recordId);
  // Auto-create a task for the follow-up
  await supabase.from('tasks').insert({
    title: `${cadenceLabel} follow-up`,
    [fkField]: recordId,
    due_date: dueDate,
    priority: 'Medium',
    completed: false,
  ...
  await supabase.from('activities').insert({
    [fkField]: recordId,
    activity_type: 'To-Do',
    subject: `${cadenceLabel} cadence set`,
    activity_date: new Date().toISOString().split('T')[0],
    notes: `Follow-up cadence set to ${cadenceLabel}. Next due: ${dueDate}`,
  }).then(() => {}).catch(() => {});
}
...
// ─── TIER ASSIGNMENT ─────────────────────────────────────────
// Maps numeric score → letter tier
// A+: 85–100  conviction play, multiple stacked catalysts
// A:  70–84   strong signal, worth active pursuit
// B+: 55–69   real interest, monitor closely
// B:  40–54   single catalyst or weak signal
// C:  <40     early/speculative tracking only
...
// ─── SCORE RECORD (all-in-one helper) ────────────────────────
// Runs calculateProbability + scoreTier → returns {score, tier, probability}
// Call this any time catalyst_tags, vacancy_status, lease_expiration, or
// last_transfer_date change on a lead or property.

export function scoreRecord(record) {
  const { rawScore, probability } = calculateProbability(record);
  const tier = scoreTier(rawScore);
  return { score: rawScore, tier, probability };
}

// ─── BUILDING SCORE ──────────────────────────────────────────
// Physical building quality score (0–100) based on 6 weighted factors.
// Exported here so it can be called from server-side batch jobs.
// The full UI component (BuildingSpecs.js) uses its own inline copy
// so it doesn't depend on this import at render time.
...
export function calculateBuildingScore(record) {
  ...
  score += ch >= 40 ? 25 : ch >= 36 ? 20 : ch >= 32 ? 15 : ch >= 28 ? 10 : ch >= 24 ? 5 : 0;

  // DH Ratio (20 pts) — dock doors per 10,000 SF
  const sf = parseInt(r.building_sf) || 0;
  const dd = parseInt(r.dock_doors) || 0;
  const dhRatio = sf > 0 ? dd / (sf / 10000) : 0;
  score += dhRatio >= 1.0 ? 20 : dhRatio >= 0.7 ? 15 : dhRatio >= 0.5 ? 10 : dhRatio >= 0.3 ? 5 : 0;

  // Truck Court (20 pts)
  const tc = parseInt(r.truck_court_depth) || 0;
  score += tc >= 185 ? 20 : tc >= 135 ? 15 : tc >= 100 ? 10 : tc > 0 ? 3 : 0;

  // Office % (15 pts)
  const op = parseInt(r.office_pct) || 0;
  score += r.office_pct != null ? (op === 0 ? 15 : op <= 5 ? 15 : op <= 10 ? 12 : op <= 15 ? 8 : op <= 25 ? 4 : 0) : 0;

  // Power (10 pts)
  const amps = parseInt(r.power_amps) || 0;
  score += amps >= 2000 ? 10 : amps >= 1200 ? 7 : amps >= 800 ? 4 : amps > 0 ? 2 : 0;

  // Vintage (10 pts)
  const yr = parseInt(r.year_built) || 0;
  score += yr >= 2015 ? 10 : yr >= 2005 ? 8 : yr >= 1995 ? 5 : yr >= 1985 ? 2 : 0;
...
// Recalculates score + tier for every lead in the DB.
// Returns { updated, errors } counts.
// Call from a settings page or admin action.

export async function recalcAllLeadScores() {
  if (!isConfigured()) return { updated: 0, errors: 0 };

  const { data: leads, error } = await supabase
    .from('leads')
    .select('id, catalyst_tags, vacancy_status, lease_expiration, last_transfer_date');
  ...
  for (let i = 0; i < leads.length; i += BATCH) {
    ...
    const updates = batch.map(lead => {
      const { score, tier, probability } = scoreRecord(lead);
      return { id: lead.id, score, tier, probability };
    });
    ...
    if (upsertErr) {
      errors += batch.length;
      console.error('Batch recalc error:', upsertErr);
    } else {
...
export async function fetchNewsKeywords() {
  ...
  if (error) { console.error('Fetch keywords error:', error); return []; }
  return data || [];
}

export async function fetchNewsArticles(limit = 50) {
  if (!isConfigured()) return [];
  const { data, error } = await supabase
    .from('news_articles')
    .select('*')
    .order('created_at', { ascending: false })
    .limit(limit);
  if (error) { console.error('Fetch articles error:', error); return []; }
  return data || [];
}

export async function saveNewsArticles(articles) {
  if (!isConfigured() || !articles?.length) return [];
  // Deduplicate by URL
  ...
  const { data, error } = await supabase.from('news_articles').insert(newArticles).select();
  if (error) { console.error('Save articles error:', error); return []; }
  return data || [];
}

export async function markArticleRead(id) {
  if (!isConfigured()) return;
  await supabase.from('news_articles').update({ read: true }).eq('id', id);
}

export async function toggleArticleStar(id, starred) {
  if (!isConfigured()) return;
  await supabase.from('news_articles').update({ starred }).eq('id', id);
}

export async function addNewsKeyword(keyword, category, priority = 'Medium', notes = null) {
  if (!isConfigured()) return null;
  const { data, error } = await supabase
    .from('news_keywords')
    .insert({ keyword, category, priority, notes })
    .select().single();
  if (error) throw error;
  return data;
}

export async function removeNewsKeyword(id) {
  if (!isConfigured()) return;
  await supabase.from('news_keywords').delete().eq('id', id);
}

export async function updateNewsKeyword(id, updates) {
  if (!isConfigured()) return;
  await supabase.from('news_keywords').update(updates).eq('id', id);
}

// ─── FILE ATTACHMENTS ───────────────────────────────────────

export async function fetchAttachments(recordType, recordId) {
  if (!isConfigured()) return [];
  const col = `${recordType}_id`;
  const { data, error } = await supabase
    .from('file_attachments')
    .select('*')
    .eq(col, recordId)
    .order('created_at', { ascending: false });
  if (error) { console.error('Fetch attachments error:', error); return []; }
  return data || [];
}

export async function uploadFile(file, recordType, recordId, category = null) {
  if (!isConfigured()) return null;
  ...
  const { data, error } = await supabase.from('file_attachments').insert(row).select().single();
  if (error) throw error;
  return data;
}

export async function addExternalLink(url, fileName, recordType, recordId, category = null) {
  if (!isConfigured()) return null;

  const source = url.includes('sharepoint.com') || url.includes('1drv.ms') || url.includes('onedrive')
    ? 'onedrive' : 'external';
...
export async function deleteAttachment(id, filePath = null) {
  if (!isConfigured()) return;
  // Delete from storage if it's an upload
  if (filePath) {
    await supabase.storage.from('attachments').remove([filePath]).catch(console.error);
  }
